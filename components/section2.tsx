    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10 grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-7 relative">
          ...
          {/* UPDATED: Added a Mask and Gradient to make the start soft */}
          <div className="hidden md:block absolute left-0 top-0 bottom-0 w-px mask-image-gradient">
             {/* 1. The Static Track (UPDATED: Gradient from transparent to gray) */}
...
const SpotlightCard = ({ data, index }: { data: TimelineData, index: number }) => {
  ...
  return (
    <div 
      ...
      // UPDATED: bg-zinc-900/90 for higher contrast (less transparency)
      // UPDATED: border-white/10 for slightly better edge definition
      ...
      <div className="relative z-30 p-6 md:p-10 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex flex-row md:flex-col gap-4 shrink-0 justify-start items-center md:items-start">
          ...
          <div className="flex flex-col pt-0 md:pt-2">
            <span className="text-[10px] font-mono uppercase tracking-widest text-neutral-400 group-hover/card:text-white transition-colors">
              {data.period}
            </span>
        ...
        <div className="flex-1">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-3">
              <span className="text-neutral-300 font-medium text-sm md:text-base">{data.company}</span>
              <span className="w-1 h-1 rounded-full bg-neutral-600" />
              <span className="px-2 py-0.5 rounded-full bg-white/10 border border-white/10 text-[10px] font-medium text-neutral-300 uppercase tracking-wide">
                {data.type}
              </span>
            </div>
          </div>

          {/* Description - UPDATED: text-neutral-200 (Brighter) */}
          <p className="text-neutral-200 leading-relaxed font-light mb-6 border-l border-white/20 pl-4 group-hover/card:border-white/40 transition-colors">
            {data.description}
          </p>

          {/* Bullet Points - UPDATED: text-neutral-300 (Brighter) */}
          <ul className="space-y-2 mb-8">
            {data.points.map((point, i) => (
               <li key={i} className="flex items-start gap-3 text-sm text-neutral-300 group-hover/card:text-white transition-colors">
                  <span className="mt-1.5 w-1 h-1 rounded-full bg-neutral-400 group-hover/card:bg-white transition-colors shadow-[0_0_5px_rgba(255,255,255,0)] group-hover/card:shadow-[0_0_5px_rgba(255,255,255,0.5)] shrink-0" />
                  {point}
               </li>
            ))}
          </ul>
          ...
          <div className="flex flex-wrap gap-2">
            {data.tech.map((t, i) => (
              <span 
                key={i} 
                // UPDATED: Slightly brighter text and background for chips
                className="px-3 py-1.5 rounded-md bg-white/[0.05] border border-white/[0.1] text-xs font-medium text-neutral-300 hover:text-white hover:bg-white/10 hover:border-white/20 transition-all duration-300 cursor-default"
              >
                {t}
              </span>
            ))}
