...
export default function Section6() {
  ...
  return (
    <section className="relative min-h-screen bg-transparent text-foreground font-sans py-12 md:py-24 lg:py-32 overflow-hidden">
        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between mb-12 md:mb-16 gap-6 md:gap-12">
          <div className="max-w-2xl">
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tighter text-foreground">
              Engineering <br className="hidden sm:block" /> <span className="text-foreground/50">Ecosystem.</span>
            </h1>
          </div>
          ...
          <div className="w-full xl:w-auto overflow-x-auto pb-4 xl:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <div className="p-1 bg-foreground/5 border border-foreground/10 rounded-full backdrop-blur-2xl flex w-max xl:w-auto gap-1 shadow-2xl">
                {FILTERS.map((filter) => (
                <button
                    key={filter.id}
                    ...
                    ${activeFilter === filter.id ? 'text-background' : 'text-foreground/60 hover:text-foreground hover:bg-foreground/5'}
                    `}
                ...
                >
                    ...
                    <motion.div
                        layoutId="pill"
                        className="absolute inset-0 bg-foreground rounded-full shadow-[0_2px_10px_rgba(0,0,0,0.2)]"
...
const BentoCard = ({ skill }: { skill: Skill }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative w-full h-full bg-background/40 border border-white/10 rounded-3xl overflow-hidden hover:border-white/20 transition-colors duration-500 cursor-pointer flex flex-col"
    >
      ...
      <div className="relative z-20 h-full p-5 md:p-6 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-start mb-6 md:mb-8">
            <div className="p-2.5 md:p-3 bg-background/80 rounded-2xl border border-foreground/5 text-foreground shadow-inner group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
              {skill.icon}
            </div>
            <span className="px-2.5 py-1 md:px-3 rounded-full bg-foreground/5 border border-foreground/5 text-[10px] font-medium text-foreground/60 uppercase tracking-wide group-hover:bg-foreground/10 transition-colors">
              {skill.tag}
            </span>
        </div>
        ...
        <div className="flex-1">
          <h3 className="text-lg md:text-xl font-semibold text-foreground mb-1 group-hover:text-emerald-400 transition-colors">
            {skill.name}
          </h3>
          <p className="text-xs md:text-sm text-foreground/50 line-clamp-2 mb-4 group-hover:text-foreground/70 transition-colors">
            {skill.description}
          </p>
            ...
            <div className="flex items-center mb-4">
               <div className="flex -space-x-2">
                 {skill.companies.map((c, i) => (
                    <div 
                        key={i} 
                        className="w-6 h-6 md:w-7 md:h-7 rounded-full border border-background bg-foreground/10 overflow-hidden relative z-0 hover:z-10 transition-all hover:scale-125 cursor-pointer"
                        title={c.name}
                    >
                       <Image 
                         src={c.logo} 
               ...
               <span className="ml-3 text-[10px] text-foreground/50 font-medium">
                  {skill.year}
               </span>
            </div>
          )}
        ...
        <div className="mt-auto pt-4 border-t border-foreground/5">
             <div className="flex justify-between items-end mb-2">
                <span className="text-[10px] text-foreground/50 font-mono">MASTERY</span>
             ...
             <div className="h-1 w-full bg-foreground/20 rounded-full overflow-hidden">
                <motion.div 
                  ...
                  className={`h-full rounded-full ${
                    skill.level > 90 ? 'bg-gradient-to-r from-emerald-500 to-green-400' : 'bg-foreground/50'
                  }`}
                />
             </div>
        </div>
